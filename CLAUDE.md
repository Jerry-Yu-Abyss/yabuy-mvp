...
- **第三份檢查 `npm run check:trade`（75 項）需要 Firebase Emulator**：驗交易狀態機與安全規則，是唯一能用「第三個登入帳號」測隔離的一層。先 `npm run emu`（需 JDK）再跑；沒開 emulator 時 `check:code` 會自動 skip，不會擋 commit。第 9 節驗 Cloud Function 的逾期記次，要用 `npm run emu:fn`（含 functions emulator）才跑得到，只開 `emu` 會註記略過
