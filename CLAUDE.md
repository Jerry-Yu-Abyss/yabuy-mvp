...
- **第三份檢查 `npm run check:trade`（49 項）需要 Firebase Emulator**：驗交易狀態機與安全規則，是唯一能用「第三個登入帳號」測隔離的一層。先 `npm run emu`（需 JDK）再跑；沒開 emulator 時 `check:code` 會自動 skip，不會擋 commit
